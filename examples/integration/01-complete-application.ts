/**
 * Complete Application Example - Using All CanL3 Features Together
 *
 * This is a realistic example showing how to use CanL3 in a real application
 */

import { CanL3Document, FileEditor } from '../../dist/index.js';
...
console.log('ðŸ“‹ Building a User Management System with CanL3\n');

// 1. Initialize Application Data
console.log('1. Initialize Application:');
const app = CanL3Document.fromJSON({
  app: {
...
const filename = 'user-management.CanL3';
app.saveSync(filename);
console.log('   Saved to:', filename);

// Check file size
const CanL3Size = (app.size() / 1024).toFixed(2);
const jsonSize = (JSON.stringify(app.toJSON()).length / 1024).toFixed(2);
console.log('   CanL3 size:', CanL3Size, 'KB');
console.log('   JSON size:', jsonSize, 'KB');
console.log('   Savings:', ((1 - parseFloat(CanL3Size) / parseFloat(jsonSize)) * 100).toFixed(1), '%');
console.log('');
...
const reloaded = CanL3Document.fromFileSync(filename);
...
console.log('  âœ“ File size optimization (CanL3 vs JSON)');
