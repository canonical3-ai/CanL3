#!/usr/bin/env node
/**
 * CanL3 Feature Coverage Test Suite
 *
 * Comprehensive test runner that validates ALL CanL3 features are working correctly.
        ...
        if (result instanceof Promise) {
            return result
                ...
                .catch((error) => {
                    ...
                    console.log(`  âŒ ${feature}: ${error.message}`);
                });
        } else {
            results.push({ category, feature, status: 'PASS' });
            console.log(`  âœ… ${feature}`);
...
async function runTests() {
    console.log('\nðŸš€ CanL3 Feature Coverage Test Suite\n');
    ...
    test('Core', 'Compact Format - Token Reduction', () => {
        ...
        const CanL3 = encodeCanL3(largeData);
        const reduction = ((json.length - CanL3.length) / json.length) * 100;
    ...
    test('Core', 'Human-Readable Output', () => {
        const CanL3 = encodeCanL3({ name: 'Test', value: 42 });
        if (!CanL3.includes('name:') || !CanL3.includes('value:')) {
    ...
    test('Core', 'Smart Encoding - Auto Delimiter Selection', () => {
        ...
        const CanL3 = encodeCanL3(dataWithCommas);

        // Should NOT use comma delimiter when data contains commas
        const lines = CanL3.split('\n');
    ...
    test('Core', 'Type Hints Support', () => {
        const CanL3 = encodeCanL3(sampleData, { includeTypes: true });

        if (!CanL3.includes('{') || !CanL3.includes(':')) {
    ...
    test('Query', 'JSONPath Queries - Basic', () => {
        const doc = new CanL3Document(sampleData);
    ...
    test('Modification', 'Bulk Operations - merge', () => {
        const doc = new CanL3Document({ a: 1 });
        doc.merge({ b: 2, c: 3 });
    ...
    test('Modification', 'Atomic File Edits', () => {
        const tempFile = path.join(__dirname, '.temp-test.CanL3');
        const doc = new CanL3Document(sampleData);
        ...
        try {
            ...
            const loaded = CanL3Document.load(tempFile);
    ...
    test('Performance', 'Stream Processing - Memory Efficiency', () => {
        ...
        const tempFile = path.join(__dirname, '.temp-large.CanL3');

        try {
            const CanL3 = encodeCanL3(largeData);
            fs.writeFileSync(tempFile, CanL3);

            const doc = CanL3Document.load(tempFile);
    ...
    test('Schema', 'Schema Definition - Basic', () => {
        const schemaFile = path.join(__dirname, 'schemas', 'users.schema.CanL3');
    ...
    test('Schema', 'Runtime Validation', () => {
        const doc = new CanL3Document(sampleData);
        const schemaPath = path.join(__dirname, 'schemas', 'users.schema.CanL3');
    ...
    test('Schema', 'Strict Mode Enforcement', () => {
        const doc = new CanL3Document(sampleData, { strict: true });
    ...
    test('Aggregation', 'count() - Count items', () => {
        const doc = new CanL3Document(aggData);
    ...
    Object.entries(byCategory).forEach(([category, stats]) => {
        ...
        if (stats.fail > 0) console.log(`  âŒ Fail: ${stats.fail}`);
    ...
    console.log('\n' + '='.repeat(60));
    console.log(totalFail === 0 ? 'âœ… ALL TESTS PASSED!' : 'âŒ SOME TESTS FAILED');
    console.log('='.repeat(60) + '\n');
