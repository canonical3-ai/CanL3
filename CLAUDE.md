...
# CLI development
npm run link               # Install CanL3 command locally for testing
```
...
- **`types.ts`** - Core type definitions and interfaces for the CanL3 format
- **`index.ts`** - Main entry point exporting public API (`encodeCanL3`, `decodeCanL3`, `encodeSmart`, `CanL3Document`)
- **`document.ts`** - CanL3Document class providing high-level document API (query, navigation, modification)
- **`encode.ts`** - JSON to CanL3 conversion with nested structure support
- **`decode.ts`** - CanL3 to JSON conversion (orchestrates modular parser)
- **`parser.ts`** - Low-level parsing utilities for CanL3 lines, headers, and delimiter detection
...
- **`repl/`** - Interactive REPL for exploring CanL3 documents
...
- **`document-browser.ts`** - Browser-compatible CanL3Document
- **`browser-core.ts`** - Core browser utilities
- **`browser-simple.ts`** - Lightweight browser API

### Key Features

- **CanL3Document API**: High-level document interface with query, navigation, and modification
...
```bash
# Encode JSON to CanL3 with smart optimization
CanL3 encode data.json --out data.CanL3 --smart --stats

# Decode CanL3 back to JSON
CanL3 decode data.CanL3 --out data.json

# Query CanL3 files with JSONPath expressions
CanL3 query data.CanL3 "users[?(@.age > 25)]"
CanL3 get data.CanL3 "users[0].email"

# Validate against schema
CanL3 validate data.CanL3 --schema schema.CanL3

# Compare token costs across different models
CanL3 stats data.json --tokenizer gpt-5

# Format CanL3 file with specific delimiter
CanL3 format data.CanL3 --delimiter "," --out formatted.CanL3
...
- **CLI**: Distributed as `dist/cli.js` with `CanL3` command
