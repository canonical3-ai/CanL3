 ...
 *
 * Run with: node --test eslint-plugin-CanL3-security/tests/rules.test.js
 */
...
describe('ESLint Security Rules', () => {
  ...
  describe('Plugin Index', async () => {
    ...
    it('should have recommended config', async () => {
      ...
      assert.ok(pluginDefault.configs.recommended.rules['CanL3-security/no-direct-property-access']);
    });
    ...
    it('should have strict config', async () => {
      ...
      assert.strictEqual(
        pluginDefault.configs.strict.rules['CanL3-security/no-direct-property-access'],
        'error'
      );
    });
  });
