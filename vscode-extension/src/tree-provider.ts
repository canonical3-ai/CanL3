...
export class CanL3TreeDataProvider implements vscode.TreeDataProvider<CanL3TreeItem> {
  private _onDidChangeTreeData = new vscode.EventEmitter<CanL3TreeItem | undefined | void>();
  ...
  getTreeItem(element: CanL3TreeItem): vscode.TreeItem {
    return element;
  }

  async getChildren(element?: CanL3TreeItem): Promise<CanL3TreeItem[]> {
    ...
    if (this.document !== editor.document) {
      ...
      try {
        ...
        this.parsedData = decodeCanL3(text);
      } catch (error) {
  ...
  private buildTreeItems(data: any, parentPath: string = ''): CanL3TreeItem[] {
    const items: CanL3TreeItem[] = [];
...
export class CanL3TreeItem extends vscode.TreeItem {
  constructor(
