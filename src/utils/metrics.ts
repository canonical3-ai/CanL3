...
function estimateTokensGPT5(text: string): number {
  ...
  // CanL3-specific optimizations
  const CanL3Patterns = text.match(/\w+\{[^}]*\}:|\d+,\s*\w+|\w+:\s*"""/g) || [];
  tokens -= CanL3Patterns.length * 0.5; // CanL3 syntax compresses efficiently
...
function estimateTokensGPT4o(text: string): number {
  ...
  // JSON structure optimization (CanL3 benefits from this)
...
export function formatMetrics(metrics: CompressionMetrics, formatName: string): string {
  return `${formatName}:
  Bytes: ${metrics.originalBytes} â†’ ${metrics.compressedBytes} (${metrics.byteSavingsPercent.toFixed(1)}% savings)
  Tokens: ${metrics.originalTokens} â†’ ${metrics.compressedTokens} (${metrics.tokenSavingsPercent.toFixed(1)}% savings)
