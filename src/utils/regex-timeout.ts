  ...
  try {
    ...
    if (elapsed > timeout) {
      ...
      if (typeof process !== 'undefined' && process.env?.NODE_ENV !== 'test') {
        ...
        console.warn(
          `[CanL3 Security Warning] Regex execution took ${elapsed}ms (limit: ${timeout}ms). ` +
...
export function safeRegexMatch(
  ...
  try {
    ...
    if (elapsed > timeout) {
      if (typeof process !== 'undefined' && process.env?.NODE_ENV !== 'test') {
        console.warn(
          `[CanL3 Security Warning] Regex match took ${elapsed}ms (limit: ${timeout}ms). ` +
          `Pattern: ${String(pattern).substring(0, 30)}...`
        );
      }
    }
...
export function safeRegexReplace(
  ...
  try {
    ...
    if (elapsed > timeout) {
      if (typeof process !== 'undefined' && process.env?.NODE_ENV !== 'test') {
        console.warn(
          `[CanL3 Security Warning] Regex replace took ${elapsed}ms (limit: ${timeout}ms). ` +
          `Pattern: ${String(pattern).substring(0, 30)}...`
        );
      }
    }
