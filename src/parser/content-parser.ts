/**
 * Content-level parsing - orchestrates parsing of entire CanL3 document
 */

import type { CanL3ParseContext, CanL3Object, CanL3ObjectHeader, CanL3ColumnDef } from '../types.js';
...
/**
 * Parse the main content of a CanL3 document
 * Orchestrates parsing of all top-level structures
 * @param content CanL3 content (without @CanL3 headers)
 * @param context Parse context
 * @returns Parsed object tree
 */
export function parseContent(content: string, context: CanL3ParseContext): CanL3Object {
  const lines = content.split('\n');
  const result: CanL3Object = {};
  let i = 0;
  ...
  while (i < lines.length) {
    ...
    if (singleLineMatch) {
      ...
      const header: CanL3ObjectHeader = { key, isArray: false, columns };
