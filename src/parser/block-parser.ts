...
import type { CanL3ParseContext, CanL3Value, CanL3Object, CanL3Array, CanL3ObjectHeader, CanL3ColumnDef, CanL3Delimiter } from '../types.js';
import { MISSING_FIELD_MARKER } from '../types.js';
import { parseObjectHeader, parseCanL3Line } from '../parser.js';
...
/**
 * Parse CanL3 line with bracket support for schema-first arrays
 * This parser properly handles quotes inside bracket notation
 */
function parseCanL3LineWithBracketSupport(line: string, delimiter: CanL3Delimiter = ","): string[] {
  // Handle empty lines
  if (!line || line.trim() === "") {
    return [];
  }
...
export function parseBlock(
  header: CanL3ObjectHeader | null,
  lines: string[],
  startIndex: number,
  context: CanL3ParseContext
): CanL3Value {
  if (!header) {
...
export function parseObjectBlock(
  ...
  const result: CanL3Object = needsSpecialHandling.length > 0
    ? Object.create(null)
    : {};
  ...
  while (lineIndex < lines.length) {
    ...
    if (trimmed.startsWith('#schema ')) {
      ...
      if (schemaMatch) {
        ...
        while (lineIndex < lines.length) {
          ...
          if (dataTrimmed) {
            const values = parseCanL3LineWithBracketSupport(dataTrimmed, context.delimiter);
            const rowObject: any = {};
    ...
    if (singleLineMatch) {
      ...
      if (hasMultipleColumns || valueHasDelimiter) {
        ...
        const nestedHeader: CanL3ObjectHeader = { key, isArray: false, columns };
    ...
    if (arrayMatch) {
      ...
      if (valuePart.includes('{')) {
        const nestedHeader: CanL3ObjectHeader = { key, isArray: true, arrayLength, columns: [] as CanL3ColumnDef[] };
        result[key] = parseSingleLineObject(nestedHeader, valuePart, context);
      } else {
        const fields = parseCanL3Line(valuePart, context.delimiter);
        const resultArray: CanL3Value[] = [];
...
export function parseArrayBlock(
  ...
  if (!header || header.columns.length === 0) {
    ...
    if (hasObjectHeaders || hasIndexedHeaders) {
          ...
          if (indexedMatch) {
            ...
            if (nestedArrayLength !== undefined) {
              if (valuePart) {
                ...
                const arrayValues = parseCanL3Line(valuePart, context.delimiter);
                const parsedArray: CanL3Value[] = [];
                    ...
                    if (nestedIndexedMatch) {
                      ...
                      if (nestedArrayLength !== undefined) {
                        if (nestedValuePart) {
                          ...
                          const arrayValues = parseCanL3Line(nestedValuePart, context.delimiter);
                          const parsedArray: CanL3Value[] = [];
                          for (const arrayValue of arrayValues) {
                            parsedArray.push(parsePrimitiveValue(arrayValue, context));
                          }
                        ...
                        if (nestedArrayLength !== undefined) {
                          if (nestedValuePart) {
                            ...
                            if (furtherNestedLines.length > 0) {
                              ...
                              const tempHeader: CanL3ObjectHeader = {
                                key: `[${nestedIndex}]`,
                                isArray: true,
                                columns: []
                              };
                              const furtherNestedArray: CanL3Value[] = [];
                              ...
                              for (let furtherLineIndex = 0; furtherLineIndex < furtherNestedLines.length; furtherLineIndex++) {
                                ...
                                if (furtherIndexedMatch) {
                                  ...
                                  if (furtherArrayLength !== undefined) {
                                    if (furtherValuePart) {
                                      const arrayValues = parseCanL3Line(furtherValuePart, context.delimiter);
                                      const parsedArray: CanL3Value[] = [];
                                      for (const arrayValue of arrayValues) {
                                        parsedArray.push(parsePrimitiveValue(arrayValue, context));
                                      }
      ...
      if (lines.length > 0) {
        ...
        if (arrayWithValuesMatch) {
          ...
          const values = parseCanL3Line(valuePart, context.delimiter);
          for (const value of values) {
            result.push(parsePrimitiveValue(value, context));
          }
        } else {
          // Parse as raw values (original behavior)
          const values = parseCanL3Line(lines[0], context.delimiter);
          for (const value of values) {
            result.push(parsePrimitiveValue(value, context));
          }
        }
    ...
    for (const line of lines) {
      ...
      const values = parseCanL3Line(trimmed, context.delimiter);
