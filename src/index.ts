/**
 * CanL3 (Token-Optimized Notation Language) - A text-first, LLM-friendly serialization format
 *
 * Main entry point exporting the core CanL3 API.
 */
...
// Re-export encodeCanL3 with the original interface for direct use
...
export { _encodeCanL3 as encodeCanL3, _decodeCanL3 as decodeCanL3, parseCanL3Line, parseHeaderLine, parseObjectHeader, detectDelimiter, inferPrimitiveType, coerceValue, isUniformObjectArray };

/**
 * Smart encode a JavaScript value to CanL3 with automatic optimization.
 *
 ...
 * @returns CanL3 formatted string optimized for minimal token usage
 ...
 * const CanL3 = encodeSmart({ message: 'Hello, World!' });
 * // Uses '|' delimiter since commas appear in content
 *
 * // With additional options
 * const CanL3 = encodeSmart(data, { includeTypes: true });
 * ```
 *
 * @since 1.0.0
 * @see encodeCanL3 - For manual delimiter control
 * @see decodeCanL3 - For decoding CanL3 back to JavaScript
 */
export function encodeSmart(input: CanL3Value, opts?: {
  delimiter?: "," | "|" | "\t" | ";";
...
// Export CanL3Document class (NEW in v0.6.0!)
export { CanL3Document, type DocumentStats } from './document.js';
...
export { CanL3REPL, type REPLOptions } from './repl/index.js';
