/**
 * CanL3Document - Main document class for CanL3 Query & Navigation API
 *
 * Provides a high-level interface for working with CanL3 documents including
 * querying, navigation, and export capabilities.
 */

import type { CanL3Value, EncodeOptions, DecodeOptions } from './types.js';
import { encodeCanL3 } from './encode.js';
import { decodeCanL3 } from './decode.js';
...
import { parseSchema, validateCanL3, type ValidationResult } from './schema/index.js';
...
/**
 * CanL3Document class - main interface for CanL3 documents
 *
 * @example
 * ```typescript
 * // Parse from CanL3 string
 * const doc = CanL3Document.parse(CanL3Text);
 *
 * // Query document
 ...
 * ```
 */
export class CanL3Document {
  private data: CanL3Value;
   ...
   */
  private constructor(data: CanL3Value) {
    this.data = data;
   ...
   *
   * @param CanL3Text - CanL3 format string
   * @param options - Decode options
   * @returns CanL3Document instance
   */
  static parse(CanL3Text: string, options?: DecodeOptions): CanL3Document {
    const data = decodeCanL3(CanL3Text, options);
    return new CanL3Document(data);
  }
  ...
  /**
   * Load a CanL3 document from a file synchronously
   *
  ...
  static fromFileSync(path: string): CanL3Document {
    ...
    return CanL3Document.parse(content);
  }

  /**
   * Load a CanL3 document from a file asynchronously
   *
   * @param path - File path
   * @returns Promise<CanL3Document>
   */
  ...
  /**
   * Load a CanL3 document from a file (alias for fromFileSync)
   *
   * @param path - File path
   * @returns CanL3Document instance
   *
   * @example
   * ```typescript
   * const doc = CanL3Document.load('data.CanL3');
   * ```
   */
  static load(path: string): CanL3Document {
    return CanL3Document.fromFileSync(path);
  }
  ...
  *deepValues(maxDepth?: number): Generator<any, void, undefined> {
  ...
  /**
   * Convert to CanL3 format string
   *
   * @param options - Encoding options
   * @returns CanL3 format string
   */
  toCanL3(options?: EncodeOptions): string {
    return encodeCanL3(this.data, options);
  }
  ...
  saveSync(path: string, options?: EncodeOptions): void {
    const CanL3 = this.toCanL3(options);
    writeFileSync(path, CanL3, 'utf-8');
  }
  ...
  async save(path: string, options?: EncodeOptions): Promise<void> {
    const CanL3 = this.toCanL3(options);
    await fs.writeFile(path, CanL3, 'utf-8');
  }
  ...
  /**
   * Get document size in bytes (CanL3 format)
   *
   * @returns Size in bytes
   */
  size(): number {
    const CanL3 = this.toCanL3();
    return Buffer.byteLength(CanL3, 'utf-8');
  }
  ...
  snapshot(): CanL3Document {
    return CanL3Document.fromJSON(JSON.parse(JSON.stringify(this.data)));
  }
   ...
   * ```
   */
  restore(snapshot: CanL3Document): this {
   ...
   *
   * @param schemaPath - Path to schema file (.schema.CanL3)
   * @returns Validation result with detailed error information
   *
   * @example
   * ```typescript
   * const result = doc.validate('schemas/users.schema.CanL3');
   * if (!result.valid) {
  ...
  validate(schemaPath: string): ValidationResult {
    ...
    return validateCanL3(this.data, schema);
  }
