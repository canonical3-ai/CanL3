/**
 * CanL3 Encoder - Converts JSON data to CanL3 format
 */

import type { CanL3Value, CanL3Object, CanL3Array, CanL3EncodeContext, CanL3Delimiter } from "./types.js";
...
/**
 * Encode a JavaScript value to CanL3 format.
 *
 * Converts JavaScript objects, arrays, and primitives to the
 * Token-Optimized Notation Language format. CanL3 provides
 ...
 * @param opts.version - CanL3 version string (default: "1.0")
 ...
 * @returns CanL3 formatted string with version header
 ...
 * ```typescript
 * // Basic encoding
 * const CanL3 = encodeCanL3({ name: 'Alice', age: 30 });
 * // Output:
 * // #version 1.0
 * // root:
 * //   name: Alice
 ...
 *
 * @since 1.0.0
 * @see decodeCanL3 - For decoding CanL3 back to JavaScript
 * @see encodeSmart - For automatic delimiter selection
 */
export function encodeCanL3(input: CanL3Value, opts: {
  ...
  const context: CanL3EncodeContext = {
    delimiter,
    includeTypes,
    version,
    indent,
 ...
 */
function encodeValue(value: CanL3Value, key: string, context: CanL3EncodeContext): string {
  if (value === null) {
    return `${key}: null`;
  }
  if (value === undefined) {
  ...
  if (typeof value === "object") {
    return encodeObject(value as CanL3Object, key, context);
  }
 ...
 */
function shouldUseCompactFormat(obj: CanL3Object, key: string, context: CanL3EncodeContext): boolean {
 ...
 */
function encodeObjectCompact(obj: CanL3Object, key: string, context: CanL3EncodeContext): string {
 ...
 */
function encodeArrayCompact(arr: CanL3Array, key: string, context: CanL3EncodeContext): string {
  if (arr.length === 0) {
    return `${key}[0]:`;
  }
  ...
  if ((isStrictlyUniform || isSemiUniform) && firstItem && typeof firstItem === "object" && !Array.isArray(firstItem)) {
    ...
    if (hasOnlyPrimitives) {
      ...
      for (const item of arr) {
        ...
        for (const col of columns) {
          const value = (item as CanL3Object)[col];
 ...
 * Encode an object
 */
function encodeObject(obj: CanL3Object, key: string, context: CanL3EncodeContext): string {
 ...
 */
function shouldUseSchemaFirstFormat(arr: CanL3Array, key: string, context: CanL3EncodeContext): boolean {
 ...
 */
function encodeArraySchemaFirst(arr: CanL3Array, key: string, context: CanL3EncodeContext): string | null {
  if (arr.length === 0) {
    return `${key}[0]:`;
  }
  ...
  const firstItem = arr[0] as CanL3Object;
 ...
 * Encode an array
 */
function encodeArray(arr: CanL3Array, key: string, context: CanL3EncodeContext): string {
  // BUG-NEW-002 FIX: Check recursion depth to prevent stack overflow
  const currentDepth = context.currentDepth ?? 0;
  const maxDepth = context.maxDepth ?? 500;
  if (currentDepth >= maxDepth) {
