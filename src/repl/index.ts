/**
 * CanL3 REPL (Read-Eval-Print Loop) (T035-T037)
 * Interactive CanL3 exploration and querying
 */

import { createInterface } from 'readline';
import { CanL3Document } from '../document.js';
...
export class CanL3REPL {
  private currentDoc: CanL3Document | null = null;
    ...
    try {
      ...
      try {
        ...
        if (filePath.endsWith('.json')) {
          this.currentDoc = CanL3Document.fromJSON(JSON.parse(content));
        } else {
          this.currentDoc = CanL3Document.parse(content);
        }

        this.currentFile = safePath;
        console.log(`âœ“ Loaded: ${safePath}`);
      } catch (error: any) {
        if (error.name === 'SecurityError' || error.message?.includes('Security')) {
          console.error(`âœ— Security Error: ${error.message}`);
          console.error(`âœ— Access denied to: ${filePath}`);
        } else {
          throw error;
        }
      }
    } catch (error) {
      console.error(`âœ— Error loading file: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  query(expression: string): any {
    if (!this.currentDoc) {
      console.error('âœ— No document loaded. Use .load <file> first.');
      return null;
    }
    ...
    try {
      ...
      console.error(`âœ— Query error: ${error instanceof Error ? error.message : String(error)}`);
      return null;
    }
  }
  ...
  private showHelp(): void {
    console.log(`
CanL3 REPL v0.8.0 - Interactive CanL3 Explorer
