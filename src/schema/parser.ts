/**
 * CanL3 Schema Language (TSL) parser
 */
...
import type { CanL3TypeHint } from '../types.js';

/**
 * Parse a CanL3 schema file
 */
export function parseSchema(content: string): CanL3Schema {
...
function parseType(typeStr: string): SchemaType {
  ...
  const primitiveTypes: CanL3TypeHint[] = ['str', 'u32', 'i32', 'f64', 'bool', 'null'];
  if (primitiveTypes.includes(baseTypeStr as CanL3TypeHint)) {
    return {
      kind: 'primitive',
      baseType: baseTypeStr as CanL3TypeHint,
      nullable
 ...
 */
export async function loadSchemaFromFile(filePath: string): Promise<CanL3Schema> {
