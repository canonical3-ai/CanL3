...
import type { CanL3Value, CanL3Object, CanL3Array } from '../types.js';

/**
 * Validate CanL3 data against a schema
 */
export function validateCanL3(data: CanL3Value, schema: CanL3Schema): ValidationResult {
  ...
  if (typeof data === 'object' && data !== null && !Array.isArray(data)) {
    for (const field of schema.rootFields) {
      const value = (data as CanL3Object)[field.name];
  ...
  if (schemaType.kind === 'primitive') {
    ...
    if (schemaType.baseType === 'list') {
      if (!Array.isArray(value)) {
        ...
        (value as CanL3Array).forEach((item, idx) => {
      ...
      if (typeof value !== 'object' || value === null || Array.isArray(value)) {
        ...
        for (const field of schemaType.fields) {
          const fieldValue = (value as CanL3Object)[field.name];
