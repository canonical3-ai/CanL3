#!/usr/bin/env node
/**
 * CanL3 CLI - Command line interface for CanL3 format
 */
...
async function main() {
  try {
    ...
    if (args.length === 1 && (args[0] === '--version' || args[0] === '-v')) {
      ...
      console.log(`ðŸ“¦ CanL3 Version: ${packageVersion}`);
      console.log(`ðŸ  Token-Optimized Notation Language`);
      console.log(`ðŸ“‹ Built: 2025-12-11`);
      console.log(`ðŸ” Production Ready with 100% Test Coverage`);
      return;
    }
    ...
    if (!commandHandler) {
      console.error(`âŒ Error: Unknown command '${command}'`);
    ...
    console.error(`âŒ Error: ${error instanceof Error ? error.message : String(error)}`);
    process.exit(1);
  }
}
