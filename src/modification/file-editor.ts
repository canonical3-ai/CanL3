...
export interface FileEditorOptions {
  ...
  /**
   * Encoding options for CanL3 output
   */
  encoding?: EncodeOptions;
}

/**
 * FileEditor provides safe in-place editing of CanL3 files
 *
 * Features:
...
export class FileEditor {
  ...
  /**
   * Open a CanL3 file for editing (async)
  ...
  static async open(filePath: string, options: FileEditorOptions = {}): Promise<FileEditor> {
    ...
    const data = decodeCanL3(content);
    return new FileEditor(validatedPath, data, content, options);
  }

  /**
   * Open a CanL3 file for editing (sync)
   * SECURITY FIX: Added path validation to prevent path traversal attacks
   */
  ...
  async save(): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        // Encode to CanL3
        const CanL3Content = encodeCanL3(this.data, this.options.encoding);

        // Write to temp file
        await fs.writeFile(tempPath, CanL3Content, 'utf-8');

        // Atomic rename
        await fs.rename(tempPath, this.filePath);

        // Update original content
        this.originalContent = CanL3Content;
      } catch (error) {
  ...
  saveSync(): void {
    ...
    try {
      ...
      // Write to temp file
      writeFileSync(tempPath, CanL3Content, 'utf-8');
   ...
   */
  isModified(): boolean {
    const current = encodeCanL3(this.data, this.options.encoding);
