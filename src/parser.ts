/**
 * Core parsing utilities for CanL3 format
 */

import type { ParserState, ParserMode, CanL3Delimiter, CanL3ObjectHeader, CanL3ColumnDef } from "./types.js";
...
/**
 * Parse a single CanL3 line into array of field values
 * Handles quoting, escaping, and triple-quotes according to spec
 * SECURITY: Now includes input validation limits (BF006)
 */
export function parseCanL3Line(line: string, delimiter: CanL3Delimiter = ","): string[] {
  // Handle empty lines
  if (!line || line.trim() === "") {
    return [];
  }
...
/**
 * Parse CanL3 header lines (starting with #)
 */
 ...
 */
export function parseObjectHeader(line: string): CanL3ObjectHeader | null {
  const trimmed = line.trim();
  if (!trimmed.endsWith(":")) {
    return null;
  }
...
/**
 * Detect delimiter from CanL3 content
 */
export function detectDelimiter(content: string): CanL3Delimiter {
