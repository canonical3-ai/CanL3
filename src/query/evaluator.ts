/**
 * Query Evaluator - Executes parsed path expressions against CanL3 documents
 *
 ...
 */
export function exists(document: CanL3Value, ast: PathNode[]): boolean {
 ...
 * Get the type of value at a path
 */
export function typeOf(document: CanL3Value, ast: PathNode[]): string | undefined {
  const evaluator = new QueryEvaluator(document, { enableCache: false });
