...
export class SimpleInteractiveStats {
  ...
  async start(filePath?: string, options: any = {}): Promise<void> {
    console.clear();
    console.log('ðŸš€ CanL3 Interactive Stats Dashboard');
    console.log('=====================================\n');
  ...
  private async analyzeAndShow(enhancedStats: EnhancedStats, filePath: string, options: any): Promise<void> {
    try {
      ...
      console.log(`CanL3: ${stats.CanL3Bytes} bytes, ${stats.CanL3Tokens} tokens`);
      console.log(`ðŸ’° Savings: ${stats.byteSavings}% bytes, ${stats.tokenSavings}% tokens`);
      console.log(`âš¡ Processing time: ${stats.processingTime}ms\n`);
    } catch (error) {
      console.error(`âŒ Error analyzing file: ${error}`);
    }
  }
  ...
  private async handleCompareFiles(enhancedStats: EnhancedStats, options: any): Promise<void> {
    ...
    if (file1.trim() && file2.trim()) {
      try {
        ...
        console.log(`ðŸ† Winner: ${winner} has better compression!\n`);
      } catch (error) {
        console.error(`âŒ Error comparing files: ${error}\n`);
      }
    }
  }
