...
import { parseSchema, validateCanL3 } from "../../schema/index.js";
import { safeReadFile } from "../utils.js";
import type { Command, CommandContext } from "../types.js";

export const ValidateCommand: Command = {
  name: "validate",
  description: "Validate CanL3 file against schema",

  async execute(context: CommandContext): Promise<void> {
    const { file, options, input } = context;

    if (!file.endsWith('.CanL3')) {
      console.error("âŒ Error: Validate command requires a .CanL3 file");
      process.exit(1);
    }

    if (!options.schema) {
      console.error("âŒ Error: --schema <file.schema.CanL3> is required");
      process.exit(1);
    }
    ...
    if (result.valid) {
      ...
      console.log(`âŒ Validation failed: ${result.errors.length} error(s) found\n`);
