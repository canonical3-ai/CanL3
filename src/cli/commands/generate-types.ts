...
export const GenerateTypesCommand: Command = {
  name: "generate-types",
  description: "Generate TypeScript types from CanL3 schema",

  async execute(context: CommandContext): Promise<void> {
    const { file, options, input } = context;

    if (!file.endsWith('.schema.CanL3')) {
      console.error("âŒ Error: generate-types requires a .schema.CanL3 file");
      process.exit(1);
    }

    if (!options.out) {
      console.error("âŒ Error: --out <file.ts> is required for generate-types");
      process.exit(1);
    }
