...
export function safeReadFile(userPath: string, preprocess: boolean = false): string {
  try {
    ...
    if (error instanceof SecurityError) {
      ...
      console.error(`âŒ Access denied to: ${userPath}`);
      process.exit(1);
    }

    // BUG-008 FIX: Enhanced error reporting for file operation failures
    if (error instanceof Error) {
      console.error(`âŒ Error reading file: ${error.message}`);
      console.error(`âŒ File: ${userPath}`);
    } else {
      console.error(`âŒ Unknown error reading file: ${userPath}`);
    }
...
export function safeWriteFile(userPath: string, content: string): void {
  try {
    ...
    if (error instanceof SecurityError) {
      console.error(`âŒ Security Error: ${error.message}`);
      console.error(`âŒ Cannot write to: ${userPath}`);
      process.exit(1);
    }

    // BUG-008 FIX: Enhanced error reporting for file operation failures
    if (error instanceof Error) {
      console.error(`âŒ Error writing file: ${error.message}`);
      console.error(`âŒ File: ${userPath}`);

      // Additional context for common write errors (Node.js system errors have code property)
      const nodeError = error as any;
      if (nodeError.code === 'EACCES') {
        console.error(`âŒ Permission denied. Check file/directory permissions.`);
      } else if (nodeError.code === 'ENOSPC') {
        console.error(`âŒ No space left on device.`);
      } else if (nodeError.code === 'EISDIR') {
        console.error(`âŒ Target is a directory, not a file.`);
      }
    } else {
      console.error(`âŒ Unknown error writing file: ${userPath}`);
    }
 ...
 * to prevent parsing conflicts with CanL3 directives and comments
 */
 ...
 * Display statistics
 */
export function displayStats(originalBytes: number, originalTokens: number, CanL3Bytes: number, CanL3Tokens: number, filename: string) {
  ...
  console.log(`\nðŸ“Š CanL3 Statistics for ${filename}`);
  ...
  console.log(`â”‚ CanL3           â”‚ ${CanL3Bytes.toString().padStart(11)} â”‚ ${CanL3Tokens.toString().padStart(11)} â”‚ ${byteSavings.padStart(9)}% â”‚`);
  console.log(`â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”´â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”´â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”´â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜`);
  console.log(`ðŸŽ¯ Byte reduction: ${byteSavings}% | ðŸ§  Token reduction: ${tokenSavings}%\n`);
}
