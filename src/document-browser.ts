/**
 * CanL3Document - Browser-Safe Version
 *
...
export class CanL3DocumentBrowser {
  ...
  static parse(CanL3Text: string, options?: DecodeOptions): CanL3DocumentBrowser {
    const data = decodeCanL3(CanL3Text, options);
    return new CanL3DocumentBrowser(data);
  }

  static fromJSON(data: any): CanL3DocumentBrowser {
    return new CanL3DocumentBrowser(data);
  }
  ...
  toCanL3(options?: EncodeOptions): string { return encodeCanL3(this.data, options); }
  size(): number { return new Blob([this.toCanL3()]).size; }
  ...
  getData(): CanL3Value { return this.data; }
  ...
  // Change tracking
  diff(other: CanL3DocumentBrowser): DiffResult { return diff(this.data, other.data); }
  diffString(other: CanL3DocumentBrowser): string { return formatDiff(this.diff(other)); }
  snapshot(): CanL3DocumentBrowser { return CanL3DocumentBrowser.fromJSON(JSON.parse(JSON.stringify(this.data))); }
