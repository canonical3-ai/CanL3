/**
 * Streaming encoder - Transform stream for encoding JSON to CanL3
 */
...
/**
 * Create a transform stream that encodes JSON chunks to CanL3 format
 */
  ...
  const stream = new Transform({
    ...
    transform(chunk: Buffer, encoding: string, callback: Function) {
      try {
        ...
        for (const line of lines) {
          ...
          try {
            const jsonData = safeJsonParse(line) as CanL3Value;
            const CanL3Output = encodeCanL3(jsonData, opts);

            // Add newline separator between blocks
            if (!isFirst) {
              this.push('\n');
            }
            this.push(CanL3Output);
            isFirst = false;
          } catch (err) {
    ...
    flush(callback: Function) {
      try {
        ...
        if (buffer.trim()) {
          const jsonData = safeJsonParse(buffer) as CanL3Value;
          const CanL3Output = encodeCanL3(jsonData, opts);
          this.push(CanL3Output);
        }
...
export async function* encodeIterator(
  ...
  for await (const item of iterable) {
    yield encodeCanL3(item, options);
  }
}
