/**
 * Streaming decoder - Transform stream for decoding CanL3 to JSON
 */
...
/**
 * Create a transform stream that decodes CanL3 chunks to JSON
 */
  ...
  const stream = new Transform({
      ...
      try {
        ...
        if (buffer.length + chunkStr.length > MAX_BUFFER_SIZE) {
          return callback(new Error(
            ...
            `This may indicate a malformed CanL3 file or a DoS attack.`
          ));
        }

        buffer += chunkStr;

        // Split by double newline (CanL3 block separator) or @CanL3 markers
        ...
        for (const block of blocks) {
          ...
          try {
            const jsonData = decodeCanL3(block, opts);
            this.push(jsonData);
          } catch (err) {
            // Invalid CanL3 block, skip or emit error
            if (opts.strict) {
    ...
    flush(callback: Function) {
      try {
        ...
        if (buffer.trim()) {
          const jsonData = decodeCanL3(buffer, opts);
          this.push(jsonData);
        }
...
export async function* decodeIterator(
  ...
  for await (const item of iterable) {
    if (item.trim()) {
      yield decodeCanL3(item, options);
    }
  }
}
