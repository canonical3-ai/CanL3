 ...
 *
 * Enables memory-efficient processing of large CanL3/JSON files
 */
...
/**
 * Stream query a CanL3 file line by line
 * SECURITY FIX: Added path validation to prevent path traversal attacks
 *
 * @param filePath - Path to CanL3 file
...
export async function* streamQuery(
  ...
  try {
    for await (const line of rl) {
      ...
      // Try to parse line as JSON or CanL3
      try {
        const data = safeJsonParse(line) as CanL3Value;
        ...
        // Not JSON, accumulate for CanL3 parsing
        // BUG-FIX-003: Check buffer size before accumulating
        if (buffer.length + line.length + 1 > MAX_BUFFER_SIZE) {
          throw new Error(
            `Buffer overflow prevented: CanL3 accumulation would exceed ${MAX_BUFFER_SIZE} bytes. ` +
    ...
    // If we have buffered content, try to parse as CanL3
    if (buffer.trim()) {
      try {
        const data = decodeCanL3(buffer);
        const result = evaluate(data, parseResult.ast);
