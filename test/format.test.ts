...
describe("format command - basic functionality", () => {
  test("should format simple CanL3 file to stdout", () => {
    const input = "#version 1.0\nusers[2]{id,name,role}:\n1,Alice,admin\n2,Bob,user";
    const filepath = createTestFile("simple.CanL3", input);
  ...
  test("should format and write to output file", () => {
    const input = "#version 1.0\nusers[2]{id,name}:\n1,Alice\n2,Bob";
    const inputPath = createTestFile("input.CanL3", input);
    const outputPath = join(TEST_DIR, "output.CanL3");
  ...
  test("should preserve data integrity after format", () => {
    ...
    const encoded = encodeCanL3(originalData);
    const filepath = createTestFile("roundtrip.CanL3", encoded);

    try {
      const formatted = execCLI(`format "${filepath}" --pretty`);
      const decoded = decodeCanL3(formatted);
...
describe("format command - indentation options", () => {
  test("should use default 2-space indentation", () => {
    ...
    const filepath = createTestFile("indent-default.CanL3", input);
  ...
  test("should use custom 4-space indentation", () => {
    const input = "#version 1.0\nuser{id,contact}:\nid: 1\ncontact{email}:\nemail: test@example.com";
    const filepath = createTestFile("indent-4.CanL3", input);
  ...
  test("should handle nested structures with proper indentation", () => {
    ...
    const encoded = encodeCanL3(nestedData);
    const filepath = createTestFile("nested.CanL3", encoded);

    try {
      const output = execCLI(`format "${filepath}" --pretty --indent 2`);

      // Should preserve nested structure
      const decoded = decodeCanL3(output);
...
describe("format command - delimiter options", () => {
  test("should preserve comma delimiter", () => {
    const input = "#version 1.0\n#delimiter ,\nusers[2]{id,name}:\n1,Alice\n2,Bob";
    const filepath = createTestFile("delimiter-comma.CanL3", input);
  ...
  test("should preserve pipe delimiter", () => {
    const input = "#version 1.0\n#delimiter |\nusers[2]{id,name}:\n1|Alice\n2|Bob";
    const filepath = createTestFile("delimiter-pipe.CanL3", input);
  ...
  test("should handle tab delimiter", () => {
    ...
    const encoded = encodeCanL3(data, { delimiter: "\t" });
    const filepath = createTestFile("delimiter-tab.CanL3", encoded);

    try {
      const output = execCLI(`format "${filepath}" --pretty --delimiter "\\t"`);
      const decoded = decodeCanL3(output, { delimiter: "\t" });
...
describe("format command - type hints", () => {
  test("should add type hints when --include-types is specified", () => {
    const input = "#version 1.0\nusers[2]{id,name}:\n1,Alice\n2,Bob";
    const filepath = createTestFile("types.CanL3", input);
...
describe("format command - edge cases", () => {
  test("should handle empty arrays", () => {
    ...
    const encoded = encodeCanL3(data);
    const filepath = createTestFile("empty-array.CanL3", encoded);
  ...
  test("should handle null values", () => {
    ...
    const encoded = encodeCanL3(data);
    const filepath = createTestFile("null-values.CanL3", encoded);
  ...
  test("should handle special characters in strings", () => {
    ...
    const encoded = encodeCanL3(data);
    const filepath = createTestFile("special-chars.CanL3", encoded);
  ...
  test("should handle multiline strings", () => {
    ...
    const encoded = encodeCanL3(data);
    const filepath = createTestFile("multiline.CanL3", encoded);
  ...
  test("should handle large arrays", () => {
    ...
    const encoded = encodeCanL3(data);
    const filepath = createTestFile("large-array.CanL3", encoded);
...
describe("format command - error handling", () => {
  ...
  test("should handle non-existent files", () => {
    const filepath = join(TEST_DIR, "nonexistent.CanL3");
  ...
  test("should handle malformed CanL3 data", () => {
    // Create a CanL3 with mismatched array size
    const malformedCanL3 = "#version 1.0\nusers[3]{id,name}:\n1,Alice\n2,Bob";
    const filepath = createTestFile("malformed.CanL3", malformedCanL3);
    ...
    try {
      ...
      // Should produce valid CanL3 output
...
describe("format command - version preservation", () => {
  test("should preserve CanL3 version", () => {
    const input = "#version 1.0\ndata{id}:\nid: 1";
    const filepath = createTestFile("version.CanL3", input);
  ...
  test("should allow version override", () => {
    const input = "#version 1.0\ndata{id}:\nid: 1";
    const filepath = createTestFile("version-override.CanL3", input);
...
describe("format command - complex real-world scenarios", () => {
  test("should format e-commerce product catalog", () => {
    const catalog = {
      products: [
        ...
        },
        {
          ...
          specs: {
            dpi: 1600,  // Use number instead of string to match CanL3 behavior
            battery: "AA",
            wireless: true
          }
        }
    ...
    const filepath = createTestFile("ecommerce.CanL3", encoded);
  ...
  test("should format user management system", () => {
    ...
    const encoded = encodeCanL3(users);
    const filepath = createTestFile("users.CanL3", encoded);
