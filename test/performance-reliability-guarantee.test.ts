 ...
 *
 * This test suite ensures CanL3 performs reliably under various stress conditions
 * including large datasets, complex operations, and edge cases.
 */

import { test, describe } from "node:test";
import assert from "node:assert";
import { encodeCanL3, decodeCanL3, CanL3Document } from "../dist/index.js";
...
describe("Performance and Reliability Guarantee", () => {
  describe("Large Dataset Performance", () => {
    test("should handle 10,000 user records efficiently", () => {
      ...
      const encoded = encodeCanL3(largeDataset);
    ...
    test("should handle complex nested queries on large datasets", () => {
      ...
      const CanL3 = encodeCanL3(mediumDataset);
  ...
  describe("Edge Cases and Boundary Conditions", () => {
    ...
    test("should handle deeply nested structures", () => {
      // Create reasonably deep nesting with unique property names to avoid CanL3 limitations
      ...
      const startTime = Date.now();
      const encoded = encodeCanL3(deepStructure);
      const encodeTime = Date.now() - startTime;
  ...
  describe("Data Integrity Guarantees", () => {
    test("should maintain perfect round-trip integrity across all data types", () => {
      ...
      for (let cycle = 0; cycle < 5; cycle++) {
        const encoded = encodeCanL3(current);
        const decoded = decodeCanL3(encoded);
    ...
    test("should maintain data integrity with all delimiters", () => {
      ...
      for (const delimiter of delimiters) {
        const encoded = encodeCanL3(conflictData, { delimiter });
        const decoded = decodeCanL3(encoded);
