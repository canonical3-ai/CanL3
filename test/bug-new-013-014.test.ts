 ...
 * BUG-NEW-014: CanL3Document validation after dynamic import in query.ts
 */
...
describe('BUG-NEW-014: CanL3Document Export Validation', () => {
  it('should have CanL3Document exported correctly', async () => {
    // Verify that the document module exports CanL3Document correctly
    const module = await import('../dist/document.js');

    assert.ok(module.CanL3Document, 'CanL3Document should be exported');
    assert.strictEqual(typeof module.CanL3Document.fromJSON, 'function',
      'CanL3Document.fromJSON should be a function');
    // Note: fromCanL3 is not a static method on CanL3Document
