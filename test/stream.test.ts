...
describe('Streaming - createEncodeStream', () => {
  it('should encode NDJSON stream to CanL3', (t, done) => {
...
describe('Streaming - createDecodeStream', () => {
  it('should decode CanL3 stream to JSON objects', (t, done) => {
    const CanL3Content = `@CanL3 v1
name: Alice
age: 30
...
age: 25`;
    ...
    const input = Readable.from([CanL3Content]);
    const results: any[] = [];
...
describe('Streaming - async iterators', () => {
  it('should encode using async iterator', async () => {
    ...
    for await (const CanL3 of encodeIterator(data, { delimiter: ',' })) {
      results.push(CanL3);
    }
  ...
  it('should decode using async iterator', async () => {
    ...
    const results: any[] = [];
    for await (const obj of decodeIterator(CanL3Blocks)) {
      results.push(obj);
    }
  ...
  it('should handle empty data', async () => {
    ...
    for await (const CanL3 of encodeIterator(data)) {
      results.push(CanL3);
    }
...
describe('Streaming - round-trip', () => {
  it('should preserve data integrity in stream round-trip', async () => {
    ...
    for await (const CanL3 of encodeIterator(originalData)) {
      encoded.push(CanL3);
    }
...
describe('Streaming - memory efficiency', () => {
  it('should handle multiple small chunks efficiently', async () => {
    ...
    const encoded: string[] = [];
    for await (const CanL3 of encodeIterator(chunks)) {
      encoded.push(CanL3);
    }
