...
describe('Comprehensive CLI Testing - All Commands and Options', () => {
  ...
  test('should show help when no arguments provided', () => {
    ...
    assert.ok(result.output.includes('CanL3 (Token-Optimized Notation Language) CLI'), 'Should show CLI help header');
  ...
  test('should show version information', () => {
    ...
    assert.ok(result.output.includes('CanL3 Version:'), 'Should show correct version');
  ...
  describe('Encode Command', () => {
    test('should encode JSON to CanL3 with basic options', () => {
    ...
    test('should encode with output file', () => {
      const jsonFile = createTempFile('test.json', testFixtures.simpleJson);
      const outputFile = join(process.cwd(), 'temp', 'output.CanL3');
  ...
  describe('Decode Command', () => {
    test('should decode CanL3 to JSON', () => {
      const CanL3Content = `#version 1.0
root:
  name: test
  value: 42`;
      const CanL3File = createTempFile('test.CanL3', CanL3Content);

      try {
        const result = runCliCommand(`decode ${CanL3File}`);
...
root:
  ...
  value: 42`;
      ...
      try {
        const result = runCliCommand(`decode ${CanL3File} --out ${outputFile}`);
...
root:
  name: test`;
      ...
      try {
        const result = runCliCommand(`decode ${CanL3File} --strict`);
  ...
  describe('Stats Command', () => {
    test('should show compression statistics for JSON', () => {
      ...
      try {
        ...
        assert.ok(result.output.includes('CanL3 Statistics'), 'Should show stats header');
    ...
    test('should show compression statistics for CanL3', () => {
      const CanL3Content = `#version 1.0
root{name:str,users:list}:
  test-app,2`;
      ...
      try {
        const result = runCliCommand(`stats ${CanL3File}`);

        assert.ok(result.success, 'Stats for CanL3 should succeed');
        assert.ok(result.output.includes('CanL3 Statistics'), 'Should show stats header');
      } finally {
        cleanupTempFile(CanL3File);
      }
    ...
    test('should work with different tokenizers', () => {
      ...
      try {
        ...
        for (const tokenizer of tokenizers) {
          ...
          assert.ok(result.output.includes('CanL3 Statistics'), `Should show stats with ${tokenizer}`);
        }
      } finally {
        cleanupTempFile(jsonFile);
      }
    ...
    test('should work with interactive mode', () => {
      ...
      try {
        ...
        assert.ok(result.output.includes('CanL3 Interactive Stats Dashboard'), 'Should show interactive header');
      } finally {
        cleanupTempFile(jsonFile);
      }
    });
...
root{name:str,value:i32}:
  test,42`;
      ...
      try {
        const result = runCliCommand(`format ${CanL3File}`);
...
root{name:str}:
  test`;
      ...
      try {
        const result = runCliCommand(`format ${CanL3File} --pretty`);
...
root{name:str}:
  test`;
      const CanL3File = createTempFile('test.CanL3', CanL3Content);
      const outputFile = join(process.cwd(), 'temp', 'formatted.CanL3');

      try {
        const result = runCliCommand(`format ${CanL3File} --out ${outputFile}`);
...
root{obj:test}:
  value:test`;
      ...
      try {
        const result = runCliCommand(`format ${CanL3File} --indent 4`);
...
root{name}:
  test`;
      ...
      try {
        const result = runCliCommand(`format ${CanL3File} --include-types`);
    ...
    test('should reject non-CanL3 files', () => {
      ...
      try {
        ...
        assert.ok(result.error?.includes('.CanL3 file'), 'Should show appropriate error');
      } finally {
        cleanupTempFile(jsonFile);
      }
    });
  ...
  describe('Error Handling', () => {
    ...
    test('should handle invalid indent value', () => {
      const CanL3File = createTempFile('test.CanL3', 'root{name}: test');

      try {
        const result = runCliCommand(`format ${CanL3File} --indent invalid`);
  ...
  describe('Advanced Features', () => {
    ...
    test('should round-trip encode/decode correctly', () => {
      ...
      const CanL3File = join(process.cwd(), 'temp', 'roundtrip.CanL3');
      const jsonFile2 = join(process.cwd(), 'temp', 'roundtrip.json');

      try {
        // Encode JSON to CanL3
        const encodeResult = runCliCommand(`encode ${jsonFile} --out ${CanL3File}`);
        assert.ok(encodeResult.success, 'Encode should succeed');

        // Decode CanL3 back to JSON
        const decodeResult = runCliCommand(`decode ${CanL3File} --out ${jsonFile2}`);
