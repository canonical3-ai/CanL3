...
describe('Bug Verification Tests', () => {
  ...
  it('BUG #2: Zero-length arrays should use declared length, not calculate from fields', () => {
    ...
    // Decode and verify
    const decoded = CanL3Document.parse(CanL3);
  ...
  it('NEW BUG-001c: Array property containing objects should round-trip', () => {
    ...
    const doc1 = CanL3Document.fromJSON(data);
    const CanL31 = doc1.toCanL3();
    const decoded1 = CanL3Document.parse(CanL31);
    ...
    const doc2 = CanL3Document.fromJSON(dataWithObjInArray);
    const CanL32 = doc2.toCanL3();
    const decoded2 = CanL3Document.parse(CanL32);
