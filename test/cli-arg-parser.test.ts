...
describe('CLI Argument Parser - Complete Coverage Tests', () => {
  ...
  test('should parse validate command', () => {
    const args = ['validate', 'data.CanL3', '--schema', 'schema.CanL3', '--strict'];
    const result = parseArgs(args);

    assert.equal(result.command, 'validate', 'Should parse validate command');
    assert.equal(result.file, 'data.CanL3', 'Should parse input file');
    assert.equal(result.options.schema, 'schema.CanL3', 'Should parse schema option');
    assert.equal(result.options.strict, true, 'Should parse strict option');
  });

  test('should parse generate-types command', () => {
    const args = ['generate-types', 'schema.CanL3', '--out', 'types.ts'];
    const result = parseArgs(args);

    assert.equal(result.command, 'generate-types', 'Should parse generate-types command');
    assert.equal(result.file, 'schema.CanL3', 'Should parse input file');
    assert.equal(result.options.out, 'types.ts', 'Should parse out option');
  });

  test('should parse query command', () => {
    const args = ['query', 'data.CanL3', 'users[?(@.age > 25)]'];
    const result = parseArgs(args);
  ...
  test('should parse get command', () => {
    const args = ['get', 'data.CanL3', 'user.profile.email'];
    const result = parseArgs(args);
  ...
  test('should parse indent option with validation', () => {
    const validArgs = ['format', 'data.CanL3', '--indent', '4'];
    ...
    try {
      const invalidArgs = ['format', 'data.CanL3', '--indent', 'invalid'];
  ...
  test('should parse multiple options together', () => {
    ...
    assert.equal(result.options.out, 'output.CanL3');
  ...
  test('should handle options with values correctly', () => {
    const optionTests = [
      { args: ['encode', 'test.json', '--out', 'output.CanL3'], option: 'out', expected: 'output.CanL3' },
      { args: ['format', 'test.CanL3', '--indent', '4'], option: 'indent', expected: 4 },
      { args: ['stats', 'test.json', '--tokenizer', 'claude-3.5'], option: 'tokenizer', expected: 'claude-3.5' },
      { args: ['validate', 'test.CanL3', '--schema', 'schema.CanL3'], option: 'schema', expected: 'schema.CanL3' }
    ];
