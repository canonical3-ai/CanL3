 ...
 * - API Reference examples (CanL3Document, encode/decode, schema, streaming)
 * - Use case examples (LLM prompts, config files)
 */

import { describe, it } from 'node:test';
import assert from 'node:assert';
import { CanL3Document, encodeCanL3, decodeCanL3, encodeSmart } from '../dist/index.js';

describe('README Examples Validation', () => {

  describe('Basic Usage Examples (Quick Start)', () => {

    it('CanL3Document.fromJSON creates document from data', () => {
      ...
      assert.ok(doc instanceof CanL3Document, 'Should create CanL3Document instance');
    ...
    it('doc.toCanL3() exports as CanL3', () => {
      const doc = CanL3Document.fromJSON({
        users: [{ id: 1, name: "Alice" }]
      });

      // From README: const CanL3 = doc.toCanL3()
      const CanL3 = doc.toCanL3();
      assert.ok(typeof CanL3 === 'string', 'Should return string');
      assert.ok(CanL3.includes('Alice'), 'Should contain Alice');
    });
  ...
  describe('Format Examples', () => {
    ...
    it('Arrays of objects encode to tabular format', () => {
      ...
      assert.ok(CanL3.includes('users'), 'Should include users key');
      assert.ok(CanL3.includes('Alice'), 'Should include Alice');
      assert.ok(CanL3.includes('Bob'), 'Should include Bob');
      assert.ok(CanL3.includes('Carol'), 'Should include Carol');
    });
    ...
    it('Nested objects encode correctly', () => {
      ...
      const CanL3 = encodeCanL3(data);
      const restored = decodeCanL3(CanL3);
  ...
  describe('Use Case Examples', () => {
    ...
    it('Configuration files use case', () => {
      ...
      const CanL3 = encodeCanL3(config);
      const restored = decodeCanL3(CanL3);
  ...
  describe('API Reference Examples', () => {
    ...
    it('CanL3Document.fromJSON() creation', () => {
      // From README API Reference
      const doc = CanL3Document.fromJSON({ test: 'data' });
      assert.ok(doc instanceof CanL3Document);
    });

    it('CanL3Document.parse() creation (README says fromCanL3)', () => {
      // From README API Reference - Note: README says fromCanL3 but actual API is parse()
      const CanL3 = `#version 1.0
name: Alice
age: 30`;
      const doc = CanL3Document.parse(CanL3);
    ...
    it('doc.delete() removes values', () => {
      const doc = CanL3Document.fromJSON({ name: 'Alice', age: 30 });
    ...
    it('doc.keys() returns key iterator', () => {
      const doc = CanL3Document.fromJSON({ a: 1, b: 2 });
  ...
  describe('Encode/Decode API Examples', () => {
    ...
    it('encodeCanL3 with default options', () => {
      const data = { hello: 'world' };
      const CanL3 = encodeCanL3(data);
      assert.ok(typeof CanL3 === 'string');
      assert.ok(CanL3.includes('hello'));
    });

    it('encodeCanL3 with custom delimiter', () => {
      // From README API Reference
    ...
    it('encodeCanL3 with type hints', () => {
      // From README API Reference
    ...
    it('encodeSmart auto-optimizes', () => {
      ...
      assert.ok(typeof CanL3 === 'string');
      assert.ok(CanL3.includes('Alice'));
    });

    it('decodeCanL3 parses CanL3 text', () => {
      // From README API Reference
      const CanL3 = `name: Alice
age: 30`;
      const data = decodeCanL3(CanL3);
      assert.strictEqual(data.name, 'Alice');
      assert.strictEqual(data.age, 30);
    });

    it('decodeCanL3 with strict mode', () => {
      // From README API Reference
      const CanL3 = `name: Alice`;
      const data = decodeCanL3(CanL3, { strict: false });
      assert.strictEqual(data.name, 'Alice');
    });
  });
  ...
  describe('Round-trip Integrity', () => {
    ...
    it('Complex nested data round-trips correctly', () => {
      ...
      const CanL3 = encodeCanL3(original);
      const restored = decodeCanL3(CanL3);
