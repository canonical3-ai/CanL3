...
describe('Core Setter - T011', () => {
  describe('Basic Set Operations', () => {
    ...
    test('should set nested property', () => {
      const doc = CanL3Document.fromJSON({ user: { profile: { age: 30 } } });
  ...
  describe('Edge Cases', () => {
    ...
    test('should update existing value', () => {
      const doc = CanL3Document.fromJSON({ count: 5 });
      doc.set('count', 10);
      assert.strictEqual(doc.get('count'), 10);
    });

    test('should set null value', () => {
      const doc = CanL3Document.fromJSON({ value: 'something' });
      doc.set('value', null);
      assert.strictEqual(doc.get('value'), null);
    });

    test('should set boolean value', () => {
      const doc = CanL3Document.fromJSON({ active: false });
      doc.set('active', true);
