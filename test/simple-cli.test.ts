...
describe('Simple CLI Testing', () => {
  ...
  test('should encode JSON to CanL3', () => {
    ...
    try {
      ...
      assert.ok(result.output.includes('CanL3 Test'), 'Should preserve data');
    } finally {
  ...
  test('should encode with output file', () => {
    ...
    try {
      ...
      assert.ok(outputContent.includes('CanL3 Test'), 'Output should contain encoded data');
    } finally {
      cleanupTempFile(jsonFile);
  ...
  test('should decode CanL3 to JSON', () => {
    ...
    try {
      ...
      assert.ok(result.output.includes('"CanL3 Test"'), 'Should decode to JSON');
  ...
  test('should show stats for CanL3', () => {
    const CanL3File = createTempFile('test.CanL3', CanL3Content);
