...
describe('Bug #2: CLI parseInt NaN validation', () => {
  test('should detect when indent option becomes NaN', () => {
    ...
    // When NaN is passed to encodeCanL3, it uses ?? 2 fallback
    const data = { nested: { a: 1, b: 2 } };
    const CanL3 = encodeCanL3(data, { indent: invalidIndent });
  ...
  test('should use default indent when NaN is provided', () => {
    ...
    // Explicitly pass NaN
    const CanL3 = encodeCanL3(data, { indent: NaN });
    console.log('CanL3 with NaN indent:', CanL3);
