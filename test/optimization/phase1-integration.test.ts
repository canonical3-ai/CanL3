...
describe('Phase 1 Integration', () => {
  describe('All Three Features Combined', () => {
    ...
    it('should handle all directives in a single CanL3 header', () => {
      ...
      // Simulate CanL3 header
      const CanL3Header = directives.join('\n');
      assert.ok(CanL3Header.length > 0);
      assert.ok(CanL3Header.includes('@colmap'));
      assert.ok(CanL3Header.includes('@dict'));
      assert.ok(CanL3Header.includes('@precision'));
    });
  });
