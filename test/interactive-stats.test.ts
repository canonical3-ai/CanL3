  ...
  describe('ðŸ”§ EnhancedStats Class Tests', () => {
    test('should analyze simple JSON file correctly', async () => {
      ...
      try {
        ...
        assert.ok(stats.CanL3Bytes > 0, 'Should have CanL3 bytes > 0');
        assert.ok(stats.CanL3Tokens > 0, 'Should have CanL3 tokens > 0');
    ...
    test('should analyze complex JSON file with nested structures', async () => {
      ...
      try {
        ...
        assert.ok(stats.CanL3Tokens > 0, 'CanL3 should be generated');
      } finally {
        cleanupTestFile(testFile);
      }
    });
    ...
    test('should analyze CanL3 files correctly', async () => {
      const testFile = createTestFile('CanL3-test.json', testFixtures.simpleJson);

      try {
        // First convert to CanL3
        const CanL3Command = `node dist/cli.js encode ${testFile} --out ${testFile.replace('.json', '.CanL3')}`;
        const CanL3Result = runCommand(CanL3Command);
        assert.equal(CanL3Result.code, 0, 'Should convert to CanL3 successfully');

        const CanL3File = testFile.replace('.json', '.CanL3');
        const stats = await new EnhancedStats().analyzeFile(CanL3File);

        assert.equal(stats.fileType, 'CanL3', 'Should detect CanL3 file type');
        assert.ok(stats.originalBytes > 0, 'Should have original bytes');
        assert.ok(stats.CanL3Tokens > 0, 'Should have CanL3 tokens');

        cleanupTestFile(CanL3File);
      } finally {
        cleanupTestFile(testFile);
      }
    });
  ...
  describe('ðŸ’» CLI Integration Tests', () => {
    ...
    test('should run normal stats command successfully', () => {
      ...
      try {
        ...
        assert.ok(result.stdout.includes('CanL3 Statistics'), 'Should show stats header');
    ...
    test('should show interactive mode suggestion', () => {
      ...
      try {
        ...
        assert.ok(result.stdout.includes('CanL3 stats'), 'Should suggest command with interactive flag');
    ...
    test('should handle different tokenizer options', () => {
      ...
      try {
        ...
        for (const tokenizer of tokenizers) {
          ...
          assert.ok(result.stdout.includes('CanL3 Statistics'), `Should show stats with ${tokenizer}`);
        }
      } finally {
        cleanupTestFile(testFile);
      }
  ...
  describe('ðŸ§ª Edge Cases and Error Handling', () => {
    test('should handle very small JSON files', async () => {
      ...
      try {
        ...
        assert.ok(stats.CanL3Bytes > 0, 'Should still have CanL3 output');
      } finally {
        cleanupTestFile(testFile);
      }
    });
    ...
    test('should handle JSON with special characters', async () => {
      ...
      try {
        ...
        assert.ok(stats.CanL3Bytes > 0, 'Should convert to CanL3 successfully');
      } finally {
        cleanupTestFile(testFile);
      }
    });
    ...
    test('should handle deeply nested JSON structures', async () => {
      ...
      try {
        ...
        assert.ok(stats.CanL3Bytes > 0, 'Should handle deep nesting');
    ...
    test('should handle JSON with arrays of various types', async () => {
      ...
      try {
        ...
        assert.ok(stats.CanL3Bytes > 0, 'Should handle arrays of mixed types');
        assert.ok(stats.CanL3Tokens > 0, 'Should count tokens correctly');
      } finally {
        cleanupTestFile(testFile);
      }
    });
    ...
    test('should handle JSON with null and undefined values', async () => {
      ...
      try {
        ...
        assert.ok(stats.CanL3Bytes > 0, 'Should convert with null values');
      } finally {
        cleanupTestFile(testFile);
      }
    });
  ...
  describe('ðŸ“Š Performance and Optimization Tests', () => {
    ...
    test('should handle compression ratio calculations correctly', async () => {
      ...
      try {
        ...
        // Compression ratio = CanL3 bytes / Original bytes
        const expectedRatio = stats.CanL3Bytes / stats.originalBytes;
    ...
    test('should calculate byte savings correctly', async () => {
      ...
      try {
        ...
        const expectedByteSavings = ((stats.originalBytes - stats.CanL3Bytes) / stats.originalBytes * 100).toFixed(1);
        assert.equal(stats.byteSavings, expectedByteSavings, 'Should calculate byte savings correctly');

        const expectedTokenSavings = ((stats.originalTokens - stats.CanL3Tokens) / stats.originalTokens * 100).toFixed(1);
