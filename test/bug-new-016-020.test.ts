...
describe('BUG-NEW-017: REPL async error handling for .load command', () => {
  it('should handle async load errors gracefully', async () => {
    const { CanL3REPL } = await import('../dist/repl/index.js');

    // Create REPL instance
    const repl = new CanL3REPL();
    ...
    try {
      ...
      await repl.load('/non/existent/path/file.CanL3');
...
describe('BUG-NEW-019: loadSchemaFromFile error handling', () => {
  it('should provide clear error for non-existent file', async () => {
    ...
    try {
      await loadSchemaFromFile('/non/existent/schema.CanL3');
  ...
  it('should throw on invalid schema content', async () => {
    const { loadSchemaFromFile } = await import('../dist/schema/parser.js');
    const tempDir = tmpdir();
    const tempFile = join(tempDir, `invalid-schema-${Date.now()}.CanL3`);
  ...
  it('should successfully load and parse schema file', async () => {
    const { loadSchemaFromFile } = await import('../dist/schema/parser.js');
    const tempDir = tmpdir();
    const tempFile = join(tempDir, `valid-schema-${Date.now()}.CanL3`);
...
describe('BUG-NEW-020: CanL3Error default-safe NODE_ENV behavior', () => {
  ...
  it('should hide detailed info when NODE_ENV is not set', async () => {
    ...
    const { CanL3Error } = await import('../dist/errors/index.js');

    const error = new CanL3Error('Test error', 10, 5, 'const x = y');
