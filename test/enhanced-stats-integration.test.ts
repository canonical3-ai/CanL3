...
describe('Enhanced Stats CLI - Integration Tests', () => {
  ...
  test('should analyze complex JSON data correctly', () => {
    ...
    try {
      ...
      assert.ok(result.output.includes('CanL3 Statistics'), 'Should show stats for complex data');
      assert.ok(result.output.includes('Original'), 'Should show original stats');
      assert.ok(result.output.includes('CanL3'), 'Should show CanL3 stats');
    } finally {
      cleanupTestFile(testFile);
    }
  });
  ...
  test('should handle large JSON files efficiently', () => {
    ...
    try {
      ...
      assert.ok(result.output.includes('CanL3 Statistics'), 'Should show stats for large data');
    } finally {
      cleanupTestFile(testFile);
    }
  });
  ...
  test('should handle different delimiters', () => {
    ...
    try {
      ...
      for (const delimiter of delimiters) {
        ...
        assert.ok(result.output.includes('CanL3 Statistics'), `Should show stats with ${delimiter}`);
      }
    } finally {
      cleanupTestFile(testFile);
    }
  ...
  test('should handle empty JSON gracefully', () => {
    ...
    try {
      ...
      assert.ok(result.output.includes('CanL3 Statistics'), 'Should show stats for empty JSON');
    } finally {
      cleanupTestFile(testFile);
    }
  });
  ...
  test('should handle JSON with special characters', () => {
    ...
    try {
      ...
      assert.ok(result.output.includes('CanL3 Statistics'), 'Should show stats with special chars');
    } finally {
      cleanupTestFile(testFile);
    }
  });
  ...
  test('should work with CanL3 files', () => {
    const jsonFile = createTestFile('CanL3-input', testData.simple);

    try {
      // First convert JSON to CanL3
      const CanL3Result = runCliCommand(`node dist/cli.js encode ${jsonFile} --out CanL3-test.CanL3`);
      assert.ok(CanL3Result.success, 'Should convert to CanL3');

      if (existsSync('CanL3-test.CanL3')) {
        // Test stats on CanL3 file
        const result = runCliCommand('node dist/cli.js stats CanL3-test.CanL3');

        assert.ok(result.success, 'Should analyze CanL3 files');
        assert.ok(result.output.includes('CanL3 Statistics'), 'Should show stats for CanL3');

        cleanupTestFile('CanL3-test.CanL3');
      }
    } finally {
  ...
  test('should provide meaningful compression results', () => {
    ...
    try {
      ...
      assert.ok(result.output.includes('CanL3'), 'Should show CanL3 size');
