...
describe("Realistic Schema-First Tests", () => {
  describe("Working Schema-First Features", () => {
    test("should encode and decode simple schema-first format correctly", () => {
      ...
      const encoded = encodeCanL3(data, { schemaFirst: true });
      const decoded = normalizeObject(decodeCanL3(encoded));
    ...
    test("should integrate with smart encoding", () => {
      ...
      const smartDecoded = normalizeObject(decodeCanL3(smartEncoded));

      // Test with regular encoding
      const regularEncoded = encodeCanL3(data, { schemaFirst: true });
      const regularDecoded = normalizeObject(decodeCanL3(regularEncoded));
  ...
  describe("Integration with CanL3 Features", () => {
    test("should work with different delimiters", () => {
      ...
      const commaEncoded = encodeCanL3(data, {
        schemaFirst: true,
        delimiter: ","
      });

      const pipeEncoded = encodeCanL3(data, {
        schemaFirst: true,
        delimiter: "|"
      });

      // Should both preserve data
      const commaDecoded = normalizeObject(decodeCanL3(commaEncoded));
      const pipeDecoded = normalizeObject(decodeCanL3(pipeEncoded));
    ...
    test("should maintain data integrity through multiple transformations", () => {
      ...
      // Original -> CanL3 (schema-first) -> JSON -> CanL3 (schema-first) -> JSON
      const CanL31 = encodeCanL3(originalData, { schemaFirst: true });
      const json1 = normalizeObject(decodeCanL3(CanL31));
      const CanL32 = encodeCanL3(json1, { schemaFirst: true });
      const json2 = normalizeObject(decodeCanL3(CanL32));
