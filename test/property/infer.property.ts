...
describe('Type Inference Property Tests', () => {
  ...
  describe('Round-Trip Type Preservation', () => {
    ...
    it('numbers preserve their type through encoding', async () => {
      await fc.assert(
        fc.property(
          fc.double({ noNaN: true, noDefaultInfinity: true, min: -1e6, max: 1e6 }),
          (num) => {
            const encoded = encodeCanL3({ value: num });
            const decoded = decodeCanL3(encoded) as { value: number };
    ...
    it('booleans preserve their type through encoding', async () => {
      await fc.assert(
        fc.property(fc.boolean(), (bool) => {
          const encoded = encodeCanL3({ value: bool });
          const decoded = decodeCanL3(encoded) as { value: boolean };
    ...
    it('null preserves through encoding', () => {
      const encoded = encodeCanL3({ value: null });
      const decoded = decodeCanL3(encoded) as { value: null };
    ...
    it('strings preserve their value through encoding', async () => {
      ...
      await fc.assert(
        fc.property(safeString, (str) => {
          const encoded = encodeCanL3({ value: str });
          const decoded = decodeCanL3(encoded) as { value: string };
    ...
    it('arrays of same type preserve types', async () => {
      await fc.assert(
        fc.property(
          ...
          (arr) => {
            const encoded = encodeCanL3({ items: arr });
            const decoded = decodeCanL3(encoded) as { items: number[] };
    ...
    it('arrays of booleans preserve types', async () => {
      await fc.assert(
        fc.property(
          ...
          (arr) => {
            const encoded = encodeCanL3({ flags: arr });
            const decoded = decodeCanL3(encoded) as { flags: boolean[] };
      ...
      await fc.assert(
        fc.property(
          ...
          (obj) => {
            ...
            const decoded = decodeCanL3(encoded) as typeof obj;
