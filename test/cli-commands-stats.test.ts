...
describe('StatsCommand - Complete Coverage Tests', () => {
  ...
  test('should have correct command properties', () => {
    ...
    assert.equal(StatsCommand.description, 'Display compression statistics for JSON or CanL3 files (supports interactive mode)', 'Should have correct description');
  ...
  test('should handle CanL3 file analysis in normal mode', async () => {
    const CanL3Content = `#version 1.0
root{name,value}:
  test,42`;

    const mockFile = createMockFile(CanL3Content);
    // Rename to .CanL3 extension
    const CanL3Path = mockFile.path.replace('.json', '.CanL3');
    const { renameSync } = await import('fs');
    renameSync(mockFile.path, CanL3Path);
