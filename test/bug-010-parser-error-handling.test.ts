...
`;
    ...
    try {
      const result = decodeCanL3(malformedCanL3, { strict: true });
...
`;
    ...
    const result = decodeCanL3(malformedCanL3, { strict: false });
...
`;
    ...
    try {
      const result = decodeCanL3(contextMalformedCanL3, { strict: true });
...
`;
    ...
    try {
      const strictResult = decodeCanL3(mixedCanL3, { strict: true });
    ...
    const nonStrictResult = decodeCanL3(mixedCanL3, { strict: false });
  ...
  await t.test('should not silently swallow TypeErrors in parsing', () => {
    // Create a CanL3 that might cause type-related parsing issues
    const edgeCaseCanL3 = `
#version 1.0
root:
...
`;
    ...
    try {
      ...
      const result = decodeCanL3(edgeCaseCanL3, { strict: true });
      if (result && typeof result === 'object') {
