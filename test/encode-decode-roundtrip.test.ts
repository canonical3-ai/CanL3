...
describe("encode/decode round-trip", () => {
  ...
  test("should round-trip array of objects", () => {
    ...
    const encoded = encodeCanL3(original, { includeTypes: true });
    const decoded = decodeCanL3(encoded);
    assert.deepStrictEqual(decoded, original);
  });
...
2, Bob, extra_field`;
    ...
    const decoded = decodeCanL3(malformed, { strict: false });
...
2, Bob, extra_field`;
    ...
    assert.throws(() => {
      decodeCanL3(malformed, { strict: true });
    });
  });

  test("should handle empty input", () => {
    const decoded = decodeCanL3("");
    assert.deepStrictEqual(decoded, {});
  });

  test("should handle headers only", () => {
    const headersOnly = "#version 1.0\n#delimiter |";
    const decoded = decodeCanL3(headersOnly);
    assert.deepStrictEqual(decoded, {});
  });
});
