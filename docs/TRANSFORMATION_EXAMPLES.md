# CanL3 Transformation Examples - Complete Guide

**Version:** 2.0.6
**Purpose:** Side-by-side JSON â†” CanL3 transformation examples
**For:** Developers implementing CanL3 in any programming language
...
**CanL3 (Short arrays - single line):**
```
#version 1.0
root{numbers,tags}:
  numbers[5]: 1, 2, 3, 4, 5
...
**CanL3 (pipe delimiter - better for this data):**
```
#version 1.0
#delimiter |
items[2]{name,price}:
...
| JSON Value | Inferred Type | CanL3 Representation |
|------------|---------------|---------------------|
| `null` | `null` | `null` |
| `true` | `bool` | `true` |
| `false` | `bool` | `false` |
...
**CanL3 Solution 1: Tabular with null**
```
#version 1.0
...
### JSON Features NOT Supported in CanL3
...
```javascript
const { encodeSmart } = require('CanL3');
const CanL3 = encodeSmart(data);
console.log(CanL3.length);  // e.g., 3200 bytes (36% savings)
```

**Step 3:** Validate round-trip
```javascript
const { decodeCanL3 } = require('CanL3');
...
// After
const { decodeCanL3 } = require('CanL3');
const data = decodeCanL3(fs.readFileSync('data.CanL3', 'utf-8'));
```
...
```
CanL3_DOCUMENT = HEADER_SECTION DATA_SECTION
...
CanL3: root{name,age}: name: Alice age: 30
...
- âœ… **Complete transformation rules** (JSON â†” CanL3)
...
**Use this as a reference when implementing CanL3 in any programming language.**
...
**For Support:**
- Open an issue: https://github.com/CanL3-dev/CanL3/issues
- Check examples: https://github.com/CanL3-dev/CanL3/tree/main/examples
- Read spec: https://github.com/CanL3-dev/CanL3/blob/main/docs/SPECIFICATION.md
