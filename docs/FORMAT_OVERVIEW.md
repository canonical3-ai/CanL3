...
CanL3 syntax reference with concrete examples. See [Getting Started](./GETTING_STARTED.md) for introduction.

## Data Model

CanL3 models data the same way as JSON:
...
A CanL3 document can represent different root forms:
...
CanL3 documents can include optional metadata headers at the top.
...
Specifies the CanL3 format version. Defaults to `1.0` if omitted.
...
CanL3 detects array structure and chooses the most efficient representation. Arrays declare their length in brackets: `[N]`.
...
When all objects in an array share the same set of primitive-valued keys, CanL3 uses tabular format:
...
> **Tip**: The array length helps LLMs validate structure. If you ask a model to generate CanL3 output, explicit lengths let you detect truncation or malformed data.

### Delimiter Options

CanL3 supports four delimiters: comma (default), pipe, tab, and semicolon:
...
> **Tip**: Tab delimiters often tokenize more efficiently than commas, especially for data with few quoted strings. Use `encodeCanL3(data, { delimiter: '\t' })` for additional token savings.
...
| `u32` | Unsigned 32-bit integer | number â‰¥ 0 | `id:u32` |
...
CanL3 quotes strings **only when necessary** to maximize token efficiency. A string must be quoted if:
...
| Input | Output |
|-------|--------|
| Finite number | Canonical decimal (e.g., `1e6` â†’ `1000000`) |
...
CanL3 offers two modes for handling problematic keys:
...
CanL3 supports comments for documentation:
...
2. **Directive lines** - Start with `@` (schema annotations like `@CanL3 v1`)
