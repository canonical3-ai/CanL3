# CanL3 Real-World Use Cases

This document showcases real-world scenarios where CanL3 excels.
...
```typescript
import { CanL3Document, FileEditor } from 'CanL3';
...
```typescript
// Load user data
const data = await CanL3Document.fromFile('users.CanL3');
...
```typescript
// Load old data
const oldData = await CanL3Document.fromFile('old-format.CanL3');

// Create new structure
const newData = CanL3Document.fromJSON({
  version: '2.0',
...
// Save new format
await newData.save('new-format.CanL3');
...
```typescript
import { streamQuery, streamAggregate, StreamPipeline } from 'CanL3';

// Process 50GB log file with <100MB memory
const errorCount = await streamAggregate(
  'application.log.CanL3',
  'logs[*]',
...
for await (const log of streamQuery('application.log.CanL3', 'logs[*]', {
...
for await (const error of pipeline.execute('logs.CanL3', 'logs[*]')) {
  sendAlert(error);
}
```
...
const catalog = await CanL3Document.fromFile('products.CanL3');
...
// Save with backup
const editor = await FileEditor.open('products.CanL3', { backup: true });
...
```typescript
class APICache {
  private cache: CanL3Document;

  constructor() {
    this.cache = CanL3Document.fromJSON({
...
class TestFixtures {
  private fixtures: CanL3Document;

  constructor() {
    this.fixtures = CanL3Document.fromJSON({
      users: [
...
// Encode to CanL3 (32-45% smaller)
const CanL3Format = encodeCanL3(userData, { smart: true });
const jsonFormat = JSON.stringify(userData);

console.log('JSON size:', jsonFormat.length, 'characters');
console.log('CanL3 size:', CanL3Format.length, 'characters');
console.log('Token savings:', ((1 - CanL3Format.length / jsonFormat.length) * 100).toFixed(1), '%');
...
// Parse LLM response (if it returns CanL3)
const result = CanL3Document.parse(response);
```
...
```typescript
import { streamQuery, StreamPipeline } from 'CanL3';
...
// Process pipeline
const results = [];
for await (const record of pipeline.execute('raw-data.CanL3', 'records[*]')) {
  results.push(record);
...
class DataVersionControl {
  private versions: Array<{ timestamp: Date; snapshot: CanL3Document; diff: any }> = [];
  private current: CanL3Document;

  constructor(initial: any) {
    this.current = CanL3Document.fromJSON(initial);
  ...
  modify(updateFn: (doc: CanL3Document) => void) {
...
**Build amazing things with CanL3! ðŸš€**
