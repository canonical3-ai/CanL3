# CanL3 Implementation Reference Guide

**Version:** 2.0.6
**Target Audience:** Language implementers (Python, Go, Rust, Java, C#, etc.)
**Purpose:** Complete specification for implementing CanL3 encoder/decoder in any language
...
2. [JSON to CanL3 Transformation](#json-to-CanL3-transformation)
3. [CanL3 to JSON Parsing](#CanL3-to-json-parsing)
...
CanL3 (Token-Optimized Notation Language) is a text-based serialization format designed for:
...
3. Apply appropriate encoding strategy:
   - Primitive â†’ key-value pair
   - Object â†’ block with fields
   - Array â†’ tabular or nested format
...
```python
function encodeCanL3(data, options):
  # 1. Initialize
...
```python
function parseCanL3Line(line, delimiter):
  fields = []
  current_field = ""
...
**Step 1:** Identify type â†’ Object
**Step 2:** Extract keys â†’ ["active", "id", "name"] (sorted)
**Step 3:** Check for nesting â†’ No nested objects/arrays
**Step 4:** Generate header â†’ `root{active,id,name}:`
...
- "Alice Smith" has space? Yes â†’ Quote? No, space is internal
...
**Step 1:** Identify array â†’ Uniform object array
**Step 2:** Columns â†’ ["name", "price"]
**Step 3:** Header â†’ `items[2]{name,price}:`
**Step 4:** Format row 1:
- name: "Item, A" â†’ Contains delimiter (`,`) â†’ **Must quote**: `"Item, A"`
- price: 99.99 â†’ Number â†’ No quote: `99.99`

**Step 5:** Format row 2:
- name: "Item B" â†’ No special chars â†’ No quote: `Item B`
- price: 149.99 â†’ Number â†’ No quote: `149.99`
...
| Type | Description | Range | JSON Equivalent | CanL3 Syntax |
|------|-------------|-------|-----------------|-------------|
...
- `""` â†’ empty string
- `null` â†’ null value
- Missing field â†’ undefined (in strict mode: error)
...
**Must throw error:** Circular references are not supported in CanL3.
...
**Rules:**
- Leading/trailing spaces â†’ Must quote
- Tabs â†’ Must quote (if tab is not delimiter)
- Newlines â†’ Must use triple quotes
...
**Note:** Keys are strings in JSON, remain strings in CanL3.
...
```
FUNCTION encode_CanL3(data, options):
...
```
FUNCTION decode_CanL3(text, options):
...
Any CanL3 implementation MUST pass these tests:
...
for delimiter in [",", "|", "\t", ";"]:
  input = { data: ["a", "b", "c"] }
  CanL3 = encode(input, {delimiter: delimiter})
  output = decode(CanL3)
  assert(output == input)

// Test 5: Auto-detection
CanL3_comma = "items[3]: a, b, c"
CanL3_pipe = "items[3]: a | b | c"
assert(detect_delimiter(CanL3_comma) == ",")
assert(detect_delimiter(CanL3_pipe) == "|")
```
...
CanL3 = encode(input, {delimiter: ","})
assert(CanL3.includes('"Hello, world"'))
output = decode(CanL3)
...
**CanL3 Output (with smart encoding - pipe delimiter):**
```
#version 1.0
#delimiter |
store{name,products}:
...
@dataclass
class CanL3ColumnDef:
    name: str
    type_hint: Optional[str] = None

@dataclass
class CanL3ObjectHeader:
    key: str
    columns: List[CanL3ColumnDef]
    is_array: bool = False
...
def parse_object_header(line: str) -> Optional[CanL3ObjectHeader]:
...
type CanL3Header struct {
    Version   string
    Delimiter string
}

type CanL3ColumnDef struct {
    Name string
    Type string
}
...
func (e *CanL3Encoder) Encode(data interface{}) (string, error) {
...
#[derive(Debug)]
pub struct CanL3Header {
    version: String,
    delimiter: char,
}

impl CanL3Encoder {
    pub fn encode(&mut self, value: &JsonValue) -> Result<String, CanL3Error> {
    ...
    fn encode_value(&mut self, value: &JsonValue, key: &str) -> Result<String, CanL3Error> {
        match value {
...
public class CanL3Encoder {
    ...
    public String encode(Object data) throws CanL3Exception {
    ...
    private String encodeValue(JsonNode node, String key) throws CanL3Exception {
        if (node.isNull()) {
            return key + ": null";
        }
        ...
        throw new CanL3Exception("Unknown node type");
    }
}
...
public class CanL3Decoder {
    private static final Pattern HEADER_PATTERN = Pattern.compile(
        "^([a-zA-Z_]\\w*)(\\[(\\d+)\\])?\\{([^}]*)\\}:$"
    );

    public Object decode(String text) throws CanL3Exception {
...
- **NPM:** https://www.npmjs.com/package/CanL3
- **Version:** 1.0.1
...
```
CanL3 Document ::= Headers DataSection
...
- **Language:** TypeScript
- **Repo:** https://github.com/CanL3-dev/CanL3
- **Tests:** 496 passing tests
- **License:** MIT

### Community
- **Issues:** https://github.com/CanL3-dev/CanL3/issues
- **Discussions:** https://github.com/CanL3-dev/CanL3/discussions
...
**A:** Map to CanL3's basic types:
- **Python:** `None` â†’ `null`, `True/False` â†’ `bool`
- **Go:** `nil` â†’ `null`, `true/false` â†’ `bool`, `int` â†’ `i32`/`f64`
- **Rust:** `Option::None` â†’ `null`, `bool` â†’ `bool`, `i32/u32` â†’ `i32/u32`
- **Java:** `null` â†’ `null`, `Boolean` â†’ `bool`, `Integer` â†’ `i32`
...
def prepare_for_CanL3(obj):
    if isinstance(obj, datetime):
        return obj.isoformat()  # Convert to string
    # ... handle other custom types
    return obj

data = prepare_for_CanL3(my_data)
CanL3 = encode(data)
```
...
To be considered a compliant CanL3 implementation, your library must:
...
CanL3 encode test/fixtures/sample.json > reference.CanL3

# Compare with your implementation
your-encoder test/fixtures/sample.json > yours.CanL3
diff reference.CanL3 yours.CanL3  # Should be identical (or semantically equivalent)

# Decode test
CanL3 decode yours.CanL3 > decoded.json
...
When implementing CanL3 in other languages:
- You MAY use this specification freely
- You SHOULD credit the original CanL3 project
- You MAY create compatible implementations
- You SHOULD link to this reference

**Suggested attribution:**
```
This library implements the CanL3 format specification.
CanL3 spec: https://github.com/CanL3-dev/CanL3/blob/main/docs/IMPLEMENTATION_REFERENCE.md
```
...
**Maintainer:** Ersin KOÃ‡ (ersinkoc@gmail.com)
**Reference Implementation:** https://github.com/CanL3-dev/CanL3
