# CanL3 Schema Language (TSL) Specification
...
CanL3 Schema Language (TSL) provides a declarative way to define data structures, validation rules, and constraints for CanL3 documents. It enables:
...
1. **Human-Readable**: Easy to read and write, similar to CanL3 itself
2. **Expressive**: Support rich validation rules and constraints
3. **Backward Compatible**: Work with existing CanL3 documents
...
```bash
# Validate CanL3 file against schema
CanL3 validate data.CanL3 --schema schema.CanL3

# Example output
âœ“ Validation successful: data.CanL3 conforms to schema
...
CanL3 validate-schema schema.CanL3
...
CanL3 generate-types schema.CanL3 --output types.ts

# With options
CanL3 generate-types schema.CanL3 \
  --output types.ts \
  --export-all \
  --readonly \
  --strict
...
```typescript
/**
 * Auto-generated from schema.CanL3
...
**Status:** Design Phase
**Feedback**: [GitHub Discussions](https://github.com/CanL3-dev/CanL3/discussions)
